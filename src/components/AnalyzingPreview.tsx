...
export const AnalyzingPreview: React.FC<AnalyzingPreviewProps> = ({
	...
	return (
		<div className="space-y-4">
			...
			<div className="relative rounded-xl overflow-hidden bg-zinc-900 border border-zinc-800 shadow-xl max-h-[380px] flex items-center justify-center">
				<img
					src={previewUrl}
					alt={filename}
					className="w-full h-full object-contain max-h-[380px] opacity-85 filter contrast-105"
				/>

				{/* Animated Laser Scanning Overlay Line */}
				<div className="absolute inset-x-0 h-1 bg-emerald-400 shadow-[0_0_15px_#10b981] animate-scan-line z-10 pointer-events-none" />

				{/* Semi-transparent grid overlay */}
				<div className="absolute inset-0 bg-emerald-950/10 pointer-events-none" />

				{/* Filename Tag */}
				<div className="absolute bottom-3 left-3 z-20 px-2.5 py-1 rounded-md bg-zinc-950/80 backdrop-blur text-[11px] font-mono text-zinc-300 border border-zinc-800">
					{filename}
				</div>
			</div>
			...
			<div className="p-4 rounded-xl bg-zinc-900/90 border border-zinc-800 space-y-3">
				...
				<div className="flex items-center gap-2 text-xs">
					<div className="flex-1 flex items-center gap-1 text-emerald-400">
						<span className="font-bold">✓</span>
						<span>Upload</span>
					</div>
					<div className="w-6 h-px bg-emerald-500/50" />
					<div className="flex-1 flex items-center gap-1 text-emerald-400 font-semibold">
						<span className="animate-spin text-emerald-400">◐</span>
						<span>Scan</span>
					</div>
					<div className="w-6 h-px bg-zinc-700" />
					<div className="flex-1 flex items-center gap-1 text-zinc-500">
						<span>○</span>
						<span>Parse</span>
					</div>
				</div>
				...
				{/* Status Message */}
				<p className="text-xs font-mono text-zinc-300 min-h-[20px] transition-all">
