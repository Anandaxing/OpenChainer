...
import type { AnalysisState, AnalysisResult, HistoryEntry } from '../types'
...
import { getLocalHistory, saveToLocalHistory, clearLocalHistory } from '../utils/history'
...
function DashboardPage() {
    ...
    setTimeout(() => {
      ...
      setTimeout(async () => {
        ...
        const result: AnalysisResult = {
          ...
          filename: typeof sourceToAnalyze === 'string' ? 'sample_schematic.svg' : sourceToAnalyze.name,
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col pcb-grid-pattern">
      <Header />
        ...
        {state !== 'idle' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-4 space-y-4 lg:sticky lg:top-20">
              {previewUrl && (
                <div className="space-y-3">
                  {state === 'analyzing' || state === 'uploading' ? (
                    <AnalyzingPreview previewUrl={previewUrl} filename={filename} />
                  ) : (
                    <div className="rounded-xl overflow-hidden bg-zinc-900 border border-zinc-800 p-2 shadow-lg">
                      <div className="relative max-h-[340px] flex items-center justify-center overflow-hidden rounded-lg bg-zinc-950">
                        <img
                          src={previewUrl}
                          alt={filename}
                      ...
                      <div className="p-3 flex items-center justify-between gap-2 border-t border-zinc-800/80 mt-2">
                        <div className="truncate">
                          <p className="text-xs font-semibold text-zinc-200 truncate">{filename}</p>
                          <p className="text-[10px] font-mono text-zinc-500">Image Loaded</p>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0">
                          <button
                            type="button"
                            onClick={resetToIdle}
                            className="px-2.5 py-1 text-xs font-medium bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded border border-zinc-700 min-h-[44px]"
                          >
                            Change
                          </button>
                        </div>
              ...
              {state === 'file_selected' && (
                <div className="fixed bottom-0 inset-x-0 p-3 bg-zinc-950/90 backdrop-blur border-t border-zinc-800 z-50 lg:static lg:bg-transparent lg:border-0 lg:p-0">
                  <button
                    type="button"
            ...
            <div className="lg:col-span-8 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto pr-1">
              {state === 'success' && analysisResult && (
                <div className="space-y-4">
                  ...
                  <div className="p-2.5 rounded-lg bg-zinc-900/60 border border-zinc-800 text-[11px] font-mono text-zinc-500 flex items-center justify-between">
                    <span>
                      Active Data Source: <code className="text-emerald-400">DUMMY_SCHEMATIC_ANALYSIS_RESULT</code>
                    </span>
                    <span className="text-zinc-600">src/dummies/mockData.ts</span>
                  </div>
                </div>
              )}
              ...
              {state === 'error' && (
                <ErrorCard
                  errorMessage={errorMessage || 'An error occurred while processing the schematic.'}
      ...
      <footer className="border-t border-zinc-900 bg-zinc-950 py-4 text-center text-xs text-zinc-500 font-mono">
        OpenChainer &copy; {new Date().getFullYear()} — Open Schematic Analysis Workbench
      </footer>
    </div>
  )
}
